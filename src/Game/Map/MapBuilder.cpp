...
MapBuilder::MapBuilder(){
	...
	cherryC = ofColor(255, 0, 0);
	strawberryC = ofColor(255, 0, 0);
	appleC = ofColor(255, 0, 0);
	pacmanSpriteSheet.load("images/CustomSheet.png");
    ...
    for (int i = 0; i < mapImage.getWidth(); i++) {
        for (int j = 0; j < mapImage.getHeight(); j++) {
            ...
            if(currentPixel == boundBoundBlock){
				...
				//Add new Green Pacman skin to the game
                Player* PacManYellow = new Player(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager, "images/pacman.png");
				Player* PacManGreen = new Player(xPos,yPos,pixelMultiplier,pixelMultiplier, entityManager, "images/Custompacman.png");
				//if(menuState->getPacmanColor()){
				// 	mapInCreation->setPlayer(PacManYellow);
				// }
				// else{
				// 	mapInCreation->setPlayer(PacManGreen);
				// }
				mapInCreation->setPlayer(PacManGreen);
